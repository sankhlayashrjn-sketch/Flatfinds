...
import { splitListingUrls } from "@/lib/listingUrls";
...
export function PreferenceForm({
  ...
  const [suggestedUrlsText, setSuggestedUrlsText] = useState("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await onSubmit(name.trim(), musts, preferences, splitListingUrls(suggestedUrlsText));
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <section className="space-y-3 border-t border-slate-200 pt-8 dark:border-slate-700">
        <div>
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">
            Found something yourself? (optional)
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Paste link(s) to any listing you found on your own — one per line. We&apos;ll read
            what we can from the page and score it alongside everything else, same musts, same
            preferences, same compromise flags if it breaks something.
          </p>
        </div>
        <textarea
          rows={3}
          value={suggestedUrlsText}
          onChange={(e) => setSuggestedUrlsText(e.target.value)}
          placeholder={"https://www.99acres.com/...\nhttps://www.nobroker.in/..."}
          className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500 dark:border-slate-600 dark:bg-slate-800"
        />
      </section>
